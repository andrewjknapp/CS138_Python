# guipoker.py

from graphics import *
from pokerapp import PokerApp
from button import Button
from cdieview import ColorDieView
from helpscreen import HelpScreen

class GraphicsInterface:

    def __init__(self):
        # Create Window
        self.win = GraphWin("Dice Poker", 600, 400)
        self.win.setBackground("green3")

        self.helpScreen = HelpScreen()

        # Create Banner
        banner = Text(Point(300,30), "Python  Poker  Parlor")
        banner.setSize(24)
        banner.setFill("yellow2")
        banner.setStyle("bold")
        banner.draw(self.win)

        self.buttons = []

        # Show Title Page
        self.showTitlePage()
       
    def showGame(self):
        self.hideTitlePage()

        self.msg = Text(Point(300,380), "Welcome to the Dice Table")
        self.msg.setSize(18)
        self.msg.draw(self.win)
        self.createDice(Point(300,100), 75)
        self.buttons = []
        self.addDiceButtons(Point(300,170), 75, 30)
        b = Button(self.win, Point(300, 230), 400, 40, "Roll Dice")
        self.buttons.append(b)
        b = Button(self.win, Point(300, 280), 150, 40, "Score")
        self.buttons.append(b)
        b = Button(self.win, Point(570,375), 40, 30, "Quit")
        self.buttons.append(b)
        b = Button(self.win, Point(30, 375), 20, 20, "i")
        self.buttons.append(b)
        self.money = Text(Point(300,325), "$100")
        self.money.setSize(18)
        self.money.draw(self.win)

    # Handles showing the title pages
    def showTitlePage(self):
        self.playButton = Button(self.win, Point(220, 100), 80, 25, "Let's Play")
        self.buttons.append(self.playButton)
        self.quitButton = Button(self.win, Point(340, 100), 80, 25, "Quit")
        self.buttons.append(self.quitButton)

    # Handles removing the title page
    def hideTitlePage(self):
        self.buttons.remove(self.playButton)
        self.playButton.undraw()
        self.buttons.remove(self.quitButton)
        self.quitButton.undraw()

    def createDice(self, center, size):
        center.move(-3*size,0)
        self.dice = []
        for i in range(5):
            view = ColorDieView(self.win, center, size)
            self.dice.append(view)
            center.move(1.5*size,0)

    def addDiceButtons(self, center, width, height):
        center.move(-3*width, 0)
        for i in range(1,6):
            label = "Die {0}".format(i)
            b = Button(self.win, center, width, height, label)
            self.buttons.append(b)
            center.move(1.5*width, 0)

    def setMoney(self, amt):
        self.money.setText("${0}".format(amt))

    def showResult(self, msg, score):
        if score > 0:
            text = "{0}! You win ${1}".format(msg, score)
        else:
            text = "You rolled {0}".format(msg)
        self.msg.setText(text)

    def setDice(self, values):
        for i in range(5):
            self.dice[i].setValue(values[i])

    def wantToPlay(self):
        while True:
            ans = self.choose(["Roll Dice", "Quit", "i"])
            self.msg.setText("")
            if (ans != "i"):
                return ans == "Roll Dice"
            else:
                if not self.helpScreen.isOpen():
                    self.helpScreen.show()
                else:
                    self.helpScreen.hide()
    
    def startGame(self):
        ans = self.choose(["Let's Play", "Quit"])
        return ans == "Let's Play"

    def close(self):
        self.win.close()

    def chooseDice(self):
        # choices is a list of the indexes of the selected dice
        choices = []                   # No dice chosen yet
        while True: 
            # wait for user to click a valid button
            b = self.choose(["Die 1", "Die 2", "Die 3", "Die 4", "Die 5",
                             "Roll Dice", "Score", "i"])

            if b[0] == "D":            # User clicked a die button
                i = int(b[4]) - 1      # Translate label to die index
                if i in choices:       # Currently selected, unselect it
                    choices.remove(i)
                    self.dice[i].setColor("black")
                else:                  # Currently unselected, select it
                    choices.append(i)
                    self.dice[i].setColor("gray")
            elif b == "i":
                if not self.helpScreen.isOpen():
                    self.helpScreen.show()
                else:
                    self.helpScreen.hide()
            else:                      # User clicked Roll or Score
                for d in self.dice:    # Revert appearance of all dice
                    d.setColor("black")
                if b == "Score":       # Score clicked, ignore choices
                    return []
                elif choices != []:    # Don't accept Roll unless some
                    return choices     #   dice are actually selected

    
    def choose(self, choices):
        buttons = self.buttons

        # activate choice buttons, deactivate others
        for b in buttons:
            if b.getLabel() in choices:
                b.activate()
            else:
                b.deactivate()

        # get mouse clicks until an active button is clicked
        while True:
            p = self.win.getMouse()
            for b in buttons:
                if b.clicked(p):
                    return b.getLabel()  # function exit here.





